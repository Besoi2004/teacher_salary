...
@section('page-title', 'Chỉnh sửa Học kỳ')
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">
                    ...
                    Chỉnh sửa Học kỳ: {{ $semester->ten_ki }} - {{ $semester->nam_hoc }}
                </h5>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="date" 
                                       ...
                                       id="ngay_bat_dau" 
                                       name="ngay_bat_dau" 
                                       value="{{ old('ngay_bat_dau', $semester->ngay_bat_dau ? $semester->ngay_bat_dau->format('Y-m-d') : '') }}" 
                                       required>
                                @error('ngay_bat_dau')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="date" 
                                       ...
                                       id="ngay_ket_thuc" 
                                       name="ngay_ket_thuc" 
                                       value="{{ old('ngay_ket_thuc', $semester->ngay_ket_thuc ? $semester->ngay_ket_thuc->format('Y-m-d') : '') }}" 
                                       required>
                                @error('ngay_ket_thuc')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-12">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('ghi_chu') is-invalid @enderror" 
                                          ...
                                          name="ghi_chu" 
                                          rows="3" 
                                          placeholder="Ghi chú thêm về học kỳ...">{{ old('ghi_chu', $semester->ghi_chu) }}</textarea>
                                @error('ghi_chu')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    ...
    // Warning when editing semester with class subjects
    @if($semester->classSubjects()->count() > 0)
        document.querySelector('form').addEventListener('submit', function(e) {
            if (!confirm('Học kỳ này đã có {{ $semester->classSubjects()->count() }} lớp học phần. Bạn có chắc chắn muốn cập nhật không?')) {
                e.preventDefault();
            }
        });
    @endif
